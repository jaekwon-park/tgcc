...
```json
{
  "hooks": {
    "Stop": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "curl -s -X POST http://localhost:47829/hooks/stop -H 'X-tgcc-Token: <TGCC_HOOK_TOKEN>' -H 'Content-Type: application/json' -d '{\"session_id\":\"$CLAUDE_SESSION_ID\",\"transcript_path\":\"$CLAUDE_TRANSCRIPT_PATH\"}'"
      }]
    }],
    "Notification": [{
      "matcher": "",
      "hooks": [{
        "type": "command",
        "command": "curl -s -X POST http://localhost:47829/hooks/notification -H 'X-tgcc-Token: <TGCC_HOOK_TOKEN>' -H 'Content-Type: application/json' -d '{\"session_id\":\"$CLAUDE_SESSION_ID\",\"message\":\"$CLAUDE_NOTIFICATION\"}'"
      }]
    }]
  }
}
