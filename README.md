...
tgcc가 Claude Code의 이벤트를 수신하려면 hook을 등록해야 합니다.
...
```json
{
  "hooks": {
    "SessionStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "curl -s -X POST http://localhost:47829/hooks/session-start -H 'X-tgcc-Token: <TGCC_HOOK_TOKEN>' -H 'Content-Type: application/json' --data-binary @-"
      }]
    }],
    "Stop": [{
      "matcher": "",
      "hooks": [{
        "type": "command",
        "command": "curl -s -X POST http://localhost:47829/hooks/stop -H 'X-tgcc-Token: <TGCC_HOOK_TOKEN>' -H 'Content-Type: application/json' --data-binary @-"
      }]
    }],
    "Notification": [{
      "matcher": "",
      "hooks": [{
        "type": "command",
        "command": "curl -s -X POST http://localhost:47829/hooks/notification -H 'X-tgcc-Token: <TGCC_HOOK_TOKEN>' -H 'Content-Type: application/json' --data-binary @-"
      }]
    }]
  }
}
...
```
/register honcho_session=topic-infra
```
