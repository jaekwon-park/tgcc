...
`{exe_dir}/hooks/` 에 토큰을 `.env`에서 자동으로 읽는 스크립트가 포함되어 있습니다:
...
```json
{
  "hooks": {
    "SessionStart": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "/opt/tgcc/bin/hooks/session-start.sh"
      }]
    }],
    "Stop": [{
      "matcher": "",
      "hooks": [{
        "type": "command",
        "command": "/opt/tgcc/bin/hooks/stop.sh"
      }]
    }],
    "Notification": [{
      "matcher": "",
      "hooks": [{
        "type": "command",
        "command": "/opt/tgcc/bin/hooks/notification.sh"
      }]
    }]
  }
}
...
> 스크립트는 `{exe_dir}/.env`의 `TGCC_HOOK_TOKEN`, `TGCC_HOOK_PORT`를 자동으로 읽습니다.
> 바이너리 경로가 다르면 스크립트 경로도 맞게 수정하세요.
